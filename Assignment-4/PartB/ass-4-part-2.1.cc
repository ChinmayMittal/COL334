...
int main (int argc, char **argv)
{
  ...
  cmd.AddValue ("printRoutingTables", "Print routing tables at 0, 10, 50, 100 and 500 seconds", printRoutingTables);
  ...
  CsmaHelper csma;
  uint32_t csmaDataRate = 5000000 ; 
  uint32_t csmaDelay = 80000 ;
  std::ofstream file_a("b.1-routing-a-" + std::to_string(csmaDelay) + ".txt");
  std::ofstream file_b("b.1-routing-b-" + std::to_string(csmaDelay) + ".txt");
  std::ofstream file_c("b.1-routing-c-" + std::to_string(csmaDelay) + ".txt");
  csma.SetChannelAttribute ("DataRate", DataRateValue (csmaDataRate));
  ...
  if (printRoutingTables)
    {
      ...
      routingHelper.PrintRoutingTableEvery (Seconds (1.0), a, Create<OutputStreamWrapper> (&file_a));
      routingHelper.PrintRoutingTableEvery (Seconds (1.0), b, Create<OutputStreamWrapper> (&file_b));
      routingHelper.PrintRoutingTableEvery (Seconds (1.0), c, Create<OutputStreamWrapper> (&file_b));
      ...
      Simulator :: Schedule( Seconds(5.0), &printSeperator) ;
      ...
      Simulator :: Schedule( Seconds(15.0), &printSeperator) ;
      ...
      Simulator :: Schedule( Seconds(55.0), &printSeperator) ;
      ...
      Simulator :: Schedule( Seconds(105.0), &printSeperator) ;
      ...
      Simulator :: Schedule( Seconds(510.0), &printSeperator) ;

      routingHelper.PrintRoutingTableAt (Seconds (550.0), a, routingStream);
      routingHelper.PrintRoutingTableAt (Seconds (550.0), b, routingStream);
      routingHelper.PrintRoutingTableAt (Seconds (550.0), c, routingStream); 
